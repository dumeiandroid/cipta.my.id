...
// Kumpulkan nama file (folder cfit) yang tidak punya pasangan di cfit1
$missingInCfit1 = [];
foreach ($filesCfit as $baseName => $dataCfit) {
    if (!isset($filesCfit1[$baseName])) {
        $missingInCfit1[] = $dataCfit['filename'];
    }
}
?>
...
<body>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 45%;">Folder: cfit1 (Pasangan)
                        <br>
                        <button onclick="downloadMissingTxt()" style="margin-top:8px;padding:6px 14px;background:#fff;color:#007bff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Download Nama File yang Tidak Ada</button>
                    </th>
                </tr>
            </thead>
            <tbody>
    ...
    <script>
        const missingInCfit1 = <?php echo json_encode($missingInCfit1); ?>;

        function downloadMissingTxt() {
            const content = missingInCfit1.join('\n');
            const blob = new Blob([content], { type: 'text/plain;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = 'file_tidak_ada_di_cfit1.txt';
