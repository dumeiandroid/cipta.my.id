...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Muat data saat halaman pertama kali dimuat (dengan filter dinamis)
    muatData();
...
/**
 * Fungsi asinkron untuk memuat data dari API contacts_filter_dinamis
 * dengan filter: id_x < 10 DAN (x_13 IS NOT NULL/TIDAK KOSONG/TIDAK 0 ATAU x_14 IS NOT NULL/TIDAK KOSONG/TIDAK 0).
 */
async function muatData() {
    ...
    try {
        // Panggil API contacts_filter_dinamis dengan parameter filter
        // URL ini menggabungkan filter:
        // - id_x kurang dari 10
        // - x_13 tidak kosong/nol
        // - x_14 tidak kosong/nol
        const apiUrl = `https://lidan-co-id.pages.dev/api/contacts_filter_dinamis?table=${tabelSaatIni}&id_x_less_than=10&x_13_tidak_kosong_atau_nol=true&x_14_tidak_kosong_atau_nol=true`;
        ...
        if (data.data && Array.isArray(data.data) && data.data.length > 0) {
            ...
            htmlIsiTabel = `<tr><td colspan="${jumlahBidangX + 2}" class="text-center py-4 text-muted">Tidak ada data (Filter: id_x &lt; 10 & (x_13 atau x_14 tidak kosong/0))</td></tr>`;
        }
