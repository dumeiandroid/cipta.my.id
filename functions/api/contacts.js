...
export async function onRequest(context) {
  ...
  try {
    switch (method) {
      case 'GET':
        return await getContacts(DB_CIPTA, tableName, corsHeaders);
      case 'POST':
        return await createContact(request, DB_CIPTA, tableName, corsHeaders);
      default:
...
async function getContacts(DB_CIPTA, tableName, corsHeaders) {
  ...
  try {
    ...
    const { results } = await DB_CIPTA.prepare(query).all();
...
async function createContact(request, DB_CIPTA, tableName, corsHeaders) {
  ...
  try {
    ...
    const result = await DB_CIPTA.prepare(query).bind(...values).run();
