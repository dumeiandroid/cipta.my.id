export async function onRequest(context) {
  ...
  if (!action || action === "version") {
    return new Response(JSON.stringify({
      version: "1.3.0", bucket: "FAM_BUCKET", status: "Protected",
  ...
  try {
    if (request.method === "GET") {
      if (!recursive) {
        ...
        do {
          ...
          const list = await env.FAM_BUCKET.list(opts);
          files.push(...list.objects.map(obj => ({
            key: obj.key, url: `https://fam.cipta.my.id/${obj.key}`,
    ...
    if (request.method === "POST") {
      ...
      if (action === "upload") {
        ...
        await env.FAM_BUCKET.put(fileName, file.stream(), {
          httpMetadata: { contentType: file.type || 'application/octet-stream', cacheControl: 'public, max-age=31536000' },
        });
        return new Response(JSON.stringify({ success: true, url: `https://fam.cipta.my.id/${fileName}` }), {
          headers: { "Content-Type": "application/json", ...corsHeaders }
        });
      }
      if (action === "rename") {
        const { oldKey, newKey, keepOriginal } = await request.json();
        const obj = await env.FAM_BUCKET.get(oldKey);
        if (!obj) return new Response(JSON.stringify({ error: "File not found" }), { status: 404, headers: corsHeaders });
        await env.FAM_BUCKET.put(newKey, obj.body, { httpMetadata: obj.httpMetadata });
        if (!keepOriginal) await env.FAM_BUCKET.delete(oldKey);
        return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
      }
      if (action === "delete") {
        const { key } = await request.json();
        await env.FAM_BUCKET.delete(key);
        return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
      }
    }
  } catch (e) {
