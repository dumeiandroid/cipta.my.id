...
export async function onRequest(context) {
  ...
  try {
    if (method === 'GET') {
      return await handleGetSingle(env.DB_KARINA, tableName, id, corsHeaders);
    } else if (method === 'PUT') {
      return await handleUpdate(request, env.DB_KARINA, tableName, id, corsHeaders);
    } else if (method === 'DELETE') {
      return await handleDelete(env.DB_KARINA, tableName, id, corsHeaders);
    } else {
...
async function handleGetSingle(DB_KARINA, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const result = await DB_KARINA.prepare(query).bind(id).first();
...
async function handleUpdate(request, DB_KARINA, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const existsResult = await DB_KARINA.prepare(existsQuery).bind(id).first();
    ...
    const updateResult = await DB_KARINA.prepare(updateQuery).bind(...updateValues).run();
...
async function handleDelete(DB_KARINA, tableName, id, corsHeaders) {
  ...
  try {
    ...
    const deleteResult = await DB_KARINA.prepare(deleteQuery).bind(id).run();
